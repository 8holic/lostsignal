...
export function buildLoreText(mission, stepIndex) {
  if (!mission) return "Enter a mission code to begin.";
...
export function getCorrectLore(step) {
  return (
    step?.correctLore ||
    step?.revealLore ||
    step?.successLore ||
...
export function checkLocation(step, distanceMeters) {
  if (!step?.target) return false;

  const radius = Number(step.target.radiusMeters ?? 20);
  return Number(distanceMeters) <= radius;
}

export function advanceStep(state) {
  if (!state?.mission || !Array.isArray(state.mission.steps)) return state;
